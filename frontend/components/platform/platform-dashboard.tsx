...
export default function PlatformDashboard({ locale }: PlatformDashboardProps) {
  ...
  const comingSoonMessage = resolveNestedMessage(messages, 'platform.comingSoonMessage');
      ...
      <header className="border-b border-gray-200 bg-white px-4 py-5 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"
      ...
      <main className="flex-1 px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4">
            {agents.map((agent) => {
              const agentHref = agent.enabled ? `${localePrefix}${agent.route}` : undefined;
              return (
                <AgentStationCard
                  key={agent.id}
                  id={agent.id}
                  ...
                  href={agentHref}
                  comingSoonMessage={agent.status === 'coming_soon' ? comingSoonMessage : undefined}
                />
              );
            })}
          </div>
