/**
 * AGENT-HARNESS-06E Step 3 — Full E2E Canary Job Submission Script
 *
 * Submits exactly ONE controlled BullMQ job for the full E2E
 * read-only file canary using the test-harness-stub adapter.
 *
 * Safety:
 ...
 *   npx tsx scripts/canary-06e-submit-job.ts --sessionId <real-session-id>
 */
...
const CANARY_PROMPT =
  'Read-only full E2E canary: list files in the controlled workspace and read README.md ' +
  'through API Gateway and container-manager. Do not write, delete, rename, install packages, ' +
  'edit env files, run browser smoke, or run validation commands. ' +
  'Return only what files were listed/read.';

const EXECUTION_ID = randomUUID();
const DB_SESSION_UUID = '00000000-06e0-4000-a000-000c060e0001';
const CONVERSATION_ID = '00000000-06e0-4000-a000-000c060e0002';
const USER_ID = 'canary-06e-user';
const API_KEY_ID = 'canary-06e-apikey';
...
function getSessionIdFromArgs(): string {
  ...
  if (idx === -1 || idx + 1 >= args.length) {
    ...
    console.error('Usage: npx tsx scripts/canary-06e-submit-job.ts --sessionId <id>');
    process.exit(1);
  }
...
async function main() {
  ...
  console.log('=== AGENT-HARNESS-06E Full E2E Canary Job Submission ===');
  ...
  try {
    await pgClient.query(
      ...
      [
        ...
        'test-harness-stub',
        'test-harness-stub',
        JSON.stringify({ canary: 'AGENT-HARNESS-06E', step: 3 }),
      ],
    );
  ...
  console.log('=== 06E canary job submitted successfully. Start the worker to pick it up. ===');
  process.exit(0);
}
