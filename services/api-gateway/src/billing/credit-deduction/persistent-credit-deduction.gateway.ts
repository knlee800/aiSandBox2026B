...
/**
 * BILLING-READY-03C1: Persistent Credit Deduction Gateway.
 *
 * Persists deduction records and mutates balance via repository layer.
 * Implements sourceEventId idempotency, overflow enforcement, and
 * balanceAfter population from actual stored balance.
 *
 * Not bound as the default gateway in 03C1 — runtime binding swap
 * is deferred to BILLING-READY-03C2.
 *
...
export class PersistentCreditDeductionGateway extends CreditDeductionGateway<Promise<CreditDeductionResult>> {
  ...
  async applyDeduction(
    ...
    const balance = await this.creditBalanceRepository.findByOwnerForUpdate(
      event.ownerId,
    );
    ...
    const appliedCredits = Math.min(totalRequestedCredits, availableBalance);
    ...
    try {
      record = await this.deductionRecordRepository.create({
        ownerId: event.ownerId,
        sourceEventId: event.sourceEventId,
    ...
    await this.creditBalanceRepository.deductBalance(balance.id, balanceAfter);
