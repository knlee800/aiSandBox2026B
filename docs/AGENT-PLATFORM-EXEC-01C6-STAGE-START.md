...
**Cleanup-contract amendment (2026-09-10, Keith-approved, EXEC-01C6A preparation):** Frozen §16 item 4 is amended. Canary `usage_records` are retained. The zero-token `credit_deduction_records` row is retained. Baseline failed jobs are untouched. Removal of a canary-owned failed BullMQ job occurs only after evidence capture and later execution authorization. The original delete-rows requirement is recorded in §16 and is **not** described as unchanged. All other acceptance criteria, including zero-token accounting, remain intact. This amendment does not authorize staging execution.
...
| Temporary data and cleanup | LIVE-11 pattern: restore `GLOBAL_EXECUTION_ENABLED=false`, stop **canary-created** session/container, do not `pm2 delete` / `pm2 kill` standing apps. Historical 06D deleted its own `usage_records` row — **inventory only, not 01C6A procedure**. EXEC-01C6A cleanup contract **AMENDED 2026-09-10** (Keith-approved): retain canary `usage_records`; retain the zero-token `credit_deduction_records` row; leave baseline failed jobs untouched; remove a canary-owned failed BullMQ job only after evidence capture and later execution authorization. Do not describe the cleanup contract as unchanged. | Exact canary-owned job/session IDs; HMAC/flag restore without leaving shared worker in a mutated state; retained ledger rows are evidence, not leftover dirt |
...
| Rollback | Restore canary-owned flags/gates/sessions; do not stop standing staging services. EXEC-01C6A cleanup **AMENDED 2026-09-10**: retain canary `usage_records` and the zero-token `credit_deduction_records` row; baseline failed jobs untouched; canary-owned failed BullMQ job removal only after evidence capture and later execution authorization. Original §16.4 delete-rows requirement is superseded by that amendment and is not unchanged. | Same; cannot un-spend provider tokens; 01C6B credit row is the authorized one-deduction |
...
4. **Cleanup contract AMENDED 2026-09-10 (Keith-approved for EXEC-01C6A).** Original frozen requirement (2026-09-07): "Delete only canary-owned `usage_records` / BullMQ jobs / ledger rows identified by the canary `executionId`." Explicit amendment: (a) canary `usage_records` are **retained**; (b) the zero-token `credit_deduction_records` row is **retained** to preserve audit and idempotency evidence; (c) baseline failed BullMQ jobs are **untouched**; (d) removal of a canary-owned failed BullMQ job occurs **only after evidence capture and later execution authorization**. Do not truncate tables. Do not erase unrelated staging records. Do not describe this contract as unchanged. Zero-token accounting AC otherwise remains intact.
