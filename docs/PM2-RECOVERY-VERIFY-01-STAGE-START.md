...
- Inputs are opened `O_RDONLY|O_NOFOLLOW` (plus `O_NONBLOCK` where the platform defines it, so that opening a FIFO with no writer returns instead of blocking; §14.3) and the opened descriptor is `fstat`-checked to be a regular file before any read; a missing file, directory, FIFO / device / other non-regular file, symlink, permission failure, or any other `OSError` on open / read → `INPUT_UNREADABLE` (exit 2; never `INTERNAL_ERROR`). The error names the input **role** (`reference`, `observation-meta`, `jlist`, `observation`) and fixed text only; the OS error string and the path are never echoed.
...
- Identifiers (`reference_id`, `observation_id`): `^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$`. Host: `^[A-Za-z0-9][A-Za-z0-9.-]{0,253}$`. App names: `^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$`. Key names: `^[A-Za-z_][A-Za-z0-9_]{0,255}$`; the key name `env` is reserved → `RESERVED_KEY_NAME`. Violations → `IDENTIFIER_FORMAT`. Every pattern in this document is a **whole-string** pattern: `$` is not the Python `re.match` end anchor (which admits a trailing LF); implementations must use `re.fullmatch` (or an equivalent full-string test) for identifiers, host, app / key names, tokens, hashes, timestamps and numeric CLI arguments (§14.3).
...
- `captured_at` ≤ `normalized_at` + 300 s (the §4.8 future-skew allowance; a Mode A run that accepted a capture up to 300 s ahead of its clock emits `normalized_at` = actual normalization time, which is then earlier than `captured_at` and must round-trip into Mode B) → else `OBSERVATION_TIMESTAMPS_INCONSISTENT` (§14.3);
...
| Mode B: `captured_at ≤ normalized_at + 300 s`; `reference_id` / key set / protected flags (§4.5) | as listed in §4.5 |
...
12. **R. Redaction gate** (`if: always()`; `id: gate`; the **only** step that writes to `$PUB`). First action: `echo "cleared=false" >> "$GITHUB_OUTPUT"; echo "gate_status=NOT_CLEARED_INCOMPLETE" >> "$GITHUB_OUTPUT"` so that a gate interrupted at any later point leaves the not-cleared default. Then, with `set -u` and every failure path ending in a **fixed** `gate_status` enum and `exit 1`:
    1. **Enumerate** `find "$EV" -mindepth 1 -print0` **into an inventory file under `$RUNNER_TEMP/gate`** (outside `$EV` and `$PUB`; never uploaded), with `find`'s stderr captured privately beside it. The enumeration itself is checked explicitly — a non-zero `find` status, any stderr output, an inventory whose last byte is not NUL (partial listing), or a reading-loop count that differs from the inventory's NUL count → `NOT_CLEARED_ENUMERATION_ERROR` (§14.3; a process-substitution exit status alone is invisible to the reading loop and is not relied upon). Zero entries → `NOT_CLEARED_EMPTY`. Any entry that is not a regular file (`-L` symlink, `-d`, device, fifo) or any entry at depth > 1 → `NOT_CLEARED_UNEXPECTED_TYPE`. Any basename not in the **fixed allowlist** `identity.txt preflight.txt tripwire.log sha256sums-verify.txt sha256-before.txt sha256-after.txt compile.txt unittest.log unittest.exit unittest.timing test-summary.json tracked-bytes.txt SUMMARY.md` → `NOT_CLEARED_UNEXPECTED_FILE`. Unreadable (`! -r`) → `NOT_CLEARED_UNREADABLE`.
...
### 14.2 Step 3 clarification record (2026-09-19; §9.1 wording only; Step 3 authorized by Keith)

Keith authorized Step 3 (bounded implementation of F1–F6) at baseline `fee4f6fffeeea1a43298d6328969e275b08f30ea` together with **one** bounded freeze clarification. The §9.1 F1 acceptance line previously read "no value / token / free text / path / argument text in class-A output", which contradicted §4.0 (echo boundary item (ii)), §5.4 (`expected_value` / `observed_value` for non-protected SET keys, display-truncated at 256) and §6.2 / §6.4 (class A "non-protected SET values"). The line now states that class-A output excludes protected values, tokens, prohibited free-text metadata, paths, and argument text, while explicitly permitted non-protected SET `value` strings of reference-declared keys remain allowed under the existing validation and display-truncation rules. No other contract, schema, code, test group, workflow rule, or acceptance criterion changed. The header status line and §12 of this document are Step 2 records and are intentionally left as frozen; current Step 3 status is mirrored on the `TASKS.md` board and in the canonical body (§7.1b), not here. Step 4 remains NOT AUTHORIZED.

### 14.3 Step 3 source-level correction record (2026-09-19; four findings; Step 4 still NOT AUTHORIZED)

Keith directed the inspection and correction of four source-level findings in the uncommitted Step 3 implementation. Corrections were written statically (no Python execution, import, byte-compilation, test or workflow run); this record names the exact contract effects.

1. **Whole-string validation.** F1 used `Pattern.match` with `$`-terminated patterns for identifiers, host, app / key names, tokens, hashes, timestamps and the numeric `--max-age-seconds` argument; Python's `$` admits one trailing LF before end-of-string, so `ref-1\n` was accepted as an identifier. Every such check now uses `fullmatch`. §4.0 records that all patterns in this document are whole-string patterns. F2 adds a T4 regression test: direct `fullmatch`-vs-`match` assertions for every pattern (`ID_RE`, `HOST_RE`, `TIMESTAMP_RE`, `HEX64_RE`, `TOKEN_RE`, `APP_RE`, `KEY_RE`, `MAX_AGE_RE`) with trailing LF / CR / space and an embedded space, plus end-to-end CLI cases — each in an isolated fixture — for a trailing-LF `reference_id`, trailing-CR `host`, trailing-LF `valid_from`, trailing-LF `jlist_sha256`, trailing-LF token, trailing-LF app name, trailing-LF key name, an embedded-space `reference_id`, and a trailing-LF `--max-age-seconds` value; each asserts the expected code and that the invalid identifier appears in neither the report nor the stderr summary. The compiled F1 literals drop the now-redundant `^` / `$` anchors (a new `MAX_AGE_RE` replaces the inline `--max-age-seconds` literal); the accepted language of every pattern, every error code and every schema are unchanged.
2. **Non-regular input.** `read_input_bytes` adds `O_NONBLOCK` (where `os` defines it; no-op elsewhere) to the open flags so that a FIFO with no writer no longer blocks the process at `open`; the existing `fstat` regular-file check then rejects it as `INPUT_UNREADABLE`. No-follow, the size bound, single-read hash binding and redaction are unchanged; `fcntl` is not imported (the flag stays set on a regular-file descriptor, which Linux ignores). §4.0 records the flag and the regular-file rule. F2 adds a POSIX-only T4 FIFO case (`os.mkfifo`, unlink in `finally`; bounded by the CLI timeout; plain return rather than `skipTest` on non-POSIX so the §8.3 no-skip parse rule is unaffected).
3. **Complete gate enumeration.** §8.3 step 12 enumerated `$EV` through `done < <(find … -print0)`, whose exit status the reading loop cannot see, and rescanned `$PUB` with `find -type f`, which hides non-regular entries. F6 now writes each `find -print0` listing to an inventory file under a new scratch directory `$RUNNER_TEMP/gate` (`GATE`; created in step 0; outside `$EV` and `$PUB`; never uploaded), captures `find`'s stderr privately beside it, and requires: `find` status 0, empty stderr, NUL-terminated inventory, and reading-loop count equal to the NUL count — for **both** `$EV` (12.1) and `$PUB` (12.4, now without `-type f`, rejecting depth > 1, symlinks, non-regular and unreadable entries). Any failure → new fixed enum `NOT_CLEARED_ENUMERATION_ERROR` (added to the step 13 enum allowlist). Successful-gate-only upload (step 14 condition) is unchanged. `enumerate` runs in the step shell, not a subshell, so `not_cleared` terminates the step.
4. **Future-skew round trip.** §4.5 required `normalized_at ≥ captured_at`, while §4.8 allows `captured_at` up to 300 s ahead of the evaluating clock. A Mode A run whose `captured_at` was 1–300 s in the future therefore emitted a normalized record (with `normalized_at` = the actual normalization time, per §4.10) that Mode B then rejected as `OBSERVATION_TIMESTAMPS_INCONSISTENT`. The Mode B rule is now `captured_at ≤ normalized_at + 300 s` (same constant `FUTURE_SKEW_SECONDS`; the permitted future window relative to the evaluating clock is unchanged; `normalized_at` is still the real normalization time, never clamped or fabricated). §4.5, §4.8, README and the F1 detail text are aligned. F2 adds T12 round-trip cases: inside the allowance (Mode A accepts, emitted record round-trips to a Mode B MATCH with identical `apps`), beyond it (Mode A `OBSERVATION_IN_FUTURE` with no record emitted; a record with `captured_at = normalized_at + 301 s` → Mode B `OBSERVATION_TIMESTAMPS_INCONSISTENT`; `+300 s` boundary accepted; `normalized_at` well after `captured_at` unaffected).

Changed implementation files: F1, F2, F3 (README), F4 (`SHA256SUMS` regenerated for the changed members), F6. F5 unchanged. r3 bundle, its manifests, root `.gitattributes`, the existing verification workflow, predecessor records and application source are byte-unchanged. Status remains `IMPLEMENTATION_WRITTEN / BEHAVIOURAL_VERIFICATION_NOT_RUN`; `EXEC-01C6A` stays `NOT_READY`; Step 4 remains NOT AUTHORIZED.

### 14.4 Step 4 attempt record (2026-09-19; single authorized dispatch; run FAILED at step 5; NOT LOCKED)

Keith authorized Step 4 against exactly `8186de3a2fc4e188fac4e0c0fe66e34b1622cccb`. Pre-dispatch: local HEAD, `origin/main` (after fetch) and the GitHub `main` branch API all equal that SHA; working tree clean; zero prior runs existed for `pm2-recovery-verify.yml` and zero runs for that SHA. One `workflow_dispatch` on `main` → **run `35427018420` attempt 1**, `head_sha` = the authorized SHA (`identity.txt`: `dispatch_sha == checkout_sha`; ubuntu-24.04; CPython 3.12.14). No retry, no redispatch.

Outcome: steps 0–4 green (`PREFLIGHT_OK`, `STATIC_ISOLATION_OK`, F1 `text` / `eol=lf` attributes honoured, no pm2 on PATH, shims installed); **step 5 (`sums`) failed with exit 2** — line 240 of F6, `listed="$(awk '{print $2}' -- "$TOOL/SHA256SUMS" | LC_ALL=C sort)"`: GNU awk treats a `--` placed after the program text as a file operand (`awk: fatal: cannot open file '--' for reading`), and under the inherited `bash -e` the failing command substitution terminated the step before the fixed `SUMS_FAIL` line. Steps 6–10 skipped; step 11 ran; the gate (step 12) enumerated 5 files, scanned 1771 bytes, `CLEARED`; step 14 uploaded `pm2-recovery-verify-35427018420-1` (six files: `identity.txt`, `preflight.txt`, `tripwire.log` (0 B), `sha256sums-verify.txt`, `SUMMARY.md`, `redaction-gate.txt` = `REDACTION_GATE_PASS files=5 scanned_bytes=1771`); steps 13 / 15 skipped (no `-FAILURE` artifact). `sha256sums-verify.txt` shows all four `sha256sum -c --strict` lines `OK`, so the manifest F4 and the committed F1 bytes agree; the runner's `VERIFIER_SHA256` line was never reached (`SUMMARY.md`: `UNAVAILABLE`). The job console carried only GitHub's script-header echo, the env listing and awk's fixed error string; no test output existed because no test ran.

§9.2 verdict: **not established** — steps 7, 8, 9, 10 did not run, so tests / parse / bytes-unchanged / `VERIFIER_SHA256` / 14-file artifact criteria have no evidence. Classification: **workflow defect in F6 step 5**, not a defect of F1, F2 or F4. Smallest next correction (not applied in this window; it is an F6 edit and therefore a re-admitted Step 3-class change followed by a newly authorized Step 4 dispatch): line 240 → `awk '{print $2}' "$TOOL/SHA256SUMS"` (the only `awk` invocation with `--`; every other `--` in F6 is git / grep / coreutils, which accept it). §11 placeholders are deliberately **not** filled from a failed run. Evidence preserved outside the repository at `C:\Users\knlee\aisb-k4-evidence\pm2-recovery-verify\run-35427018420\` (`run.json`, `jobs.json`, `artifacts.json`, `job-log.txt`, the downloaded artifact, `STEP4-VERDICT.txt`). No lock; no implementation edit; no gate weakened; occupancy EMPTY throughout (Step 4 writes no F1–F6 path, so no HOTFILE lease was needed; GOVERNANCE acquired transiently for these mirrors then released). A1 / A2 OPEN; no baseline designation, P6, P7, host CLEAN, EXEC-01C6A amendment / reopen, or canary; `EXEC-01C6A` stays `NOT_READY`.

### 14.5 F6 awk-operand correction (2026-09-19; Keith-authorized; behavioural verification still pending)

Keith authorized the bounded F6 correction identified by failed run `35427018420` at HEAD `8186de3a2fc4e188fac4e0c0fe66e34b1622cccb`. Preserved run evidence (`STEP4-VERDICT.txt`, `job-log.txt`) names the exact Step 5 failure: `awk: fatal: cannot open file '--' for reading` from `listed="$(awk '{print $2}' -- "$TOOL/SHA256SUMS" | LC_ALL=C sort)"`. Transient admission (Lane 1 ACTIVE; HOTFILE leases on the frozen F1–F6 set matching the candidate write set; candidate `status=ADMITTED` / `admissionUncertain=false`; `occupancyHash=sha256:910af2507a083e2f370f99d447be0d8e71fe65c08db06a2dd7f06dc1b36d2bc1`; validator PASS) preceded the edit; Lane 1 was released EMPTY afterwards (candidate `status=READY` / `admissionUncertain=true`; `occupancyHash=sha256:942ff6798903e6f79e92aca2e8641dfcf7d4e19903c94c3429b13f2c37e5ec3d`).

The only command changed in F6 (line 240) is:

```
FROM: listed="$(awk '{print $2}' -- "$TOOL/SHA256SUMS" | LC_ALL=C sort)"
TO:   listed="$(awk '{print $2}' "$TOOL/SHA256SUMS" | LC_ALL=C sort)"
```

Manifest set-equality (`listed` vs `git ls-files` minus `SHA256SUMS`), inherited `pipefail`, redaction, upload conditions, and every other verification gate are unchanged. F1–F5 and `SHA256SUMS` were not modified (F6 is not a manifest member). r3 and `.github/workflows/pm2-overlay-unknown-verify.yml` are untouched. F6 working-tree sha256 `aa16db1f12e343064477f3461f47643aac49724137f2c7fb5ff6bd281325e4be` (40123 B, LF, no CR). Status: F6 correction written; Attempt 1 failed before tests; behavioural verification still pending a newly authorized Step 4 dispatch. §11 placeholders remain unfilled. `EXEC-01C6A` stays `NOT_READY`. No lock.

### 14.6 Step 4 attempt 2 record (2026-09-19; single newly authorized dispatch; run FAILED at step 8 tests; NOT LOCKED)

Keith authorized one Step 4 run against exactly `c3bc92d41d122b4a8f11c32b9582c84b2e8ea898` (the commit carrying the §14.5 F6 correction). Pre-dispatch: local HEAD, `origin/main` (after fetch) and the GitHub `main` branch API all equal that SHA; working tree clean; one prior run existed for the workflow (`35427018420` @ `8186de3a…`, retained as history) and zero runs existed for the authorized SHA. One `workflow_dispatch` on `main` at 2026-09-19T07:06:01Z → **run `35428399072` attempt 1**, job `105858331384`, `head_sha` = the authorized SHA (`identity.txt`: `dispatch_sha == checkout_sha`; ubuntu-24.04, kernel 6.17.0-1022-azure; CPython 3.12.14). No retry, no redispatch.

Outcome: steps 0–7 green — the Attempt-1 defect is confirmed corrected: step 5 `sums` passed (`sha256sums-verify.txt`: four `OK` lines, `listed set equals tracked set minus SHA256SUMS`, `verifier_sha256=f3f05e57ac83034e175e115cc296a8f41f73519d23eb2618b55ffcd44def32fc`, which equals the committed F1 blob and the F4 entry); step 7 `py_compile exit=0`, `bytecode inside checkout: 0`. **Step 8 (`tests`) failed: `Ran 56 tests in 7.688s — FAILED (failures=2)`; 54 ok, 0 errors, 0 skipped; `unittest.exit=1`.** Steps 9–10 skipped; step 11 ran; the gate enumerated and scanned 10 files / 18935 bytes (`REDACTION_GATE_PASS`), and step 14 uploaded `pm2-recovery-verify-35428399072-1` (7924 B; 11 files: `compile.txt`, `identity.txt`, `preflight.txt`, `redaction-gate.txt`, `sha256-before.txt`, `sha256sums-verify.txt`, `SUMMARY.md`, `tripwire.log` (0 B), `unittest.exit`, `unittest.log`, `unittest.timing`); steps 13 / 15 skipped (no `-FAILURE` artifact). Console rule held: the job log contains no test names, `ok` / `FAIL` lines, tracebacks or `Ran N tests`; step 8 printed only GitHub's script-header echo, env names and the fixed `TESTS_FINISHED unittest_exit=1`.

The two failures are both **F2 test-authoring defects; neither contradicts F1 behaviour**:

1. `T10Bytes.test_t10_output_mode_0600_and_output_exists` (F2 line 1346) — the expected literal `["jlist.json", "normalized.json", "observation-meta.json", "report.json", "reference.json"]` is not in sorted order (`reference.json` sorts before `report.json`), so `assertEqual(sorted(os.listdir(...)), …)` fails although the directory holds exactly those five files. F1 exclusive-output / `OUTPUT_EXISTS` behaviour is as frozen.
2. `T6IdentityFreshness.test_t6_pm2_home_and_daemon_pid_checks` (F2 line 886) — `assertNotIn(".pm2", out + err)` matches the fixed report schema identifier `aisb.pm2-dual-env-comparison-report.v1`. The fixture `pm2_home` values (`/home/a/.pm2`, `/home/b/.pm2`) do not appear anywhere in stdout / stderr; the report carries only `pm2_home_declared=true` booleans and the fixed `PM2_HOME_MISMATCH` / `DAEMON_PID_MISMATCH` diagnostics (`identity.pm2_home_checked=true`, `daemon_pid_checked=true`, exit 2). The leak sentinel is over-broad; F1 `pm2_home` redaction held.

§9.2 verdict: **not established** — `unittest.exit=1`, two failures, steps 9–10 did not run (AST-count equality, `problems=[]`, `TRACKED_BYTES_UNCHANGED`, porcelain, r3 40/40 have no evidence from this run), artifact has 11 files not 14. Established by this run: dispatch identity; steps 0–7 incl. manifest set equality and `VERIFIER_SHA256` chain (`f3f05e57…32fc` == committed F1 == F4 entry == runner evidence == report `tool.self_sha256` as seen in the captured FAIL-2 output); empty tripwire; gate cleared; no `-FAILURE` artifact; console rule. Smallest next correction (NOT applied in this window; it is an F2 edit and therefore a re-admitted implementation window with HOTFILE lease, F4 `SHA256SUMS` regeneration for the F2 entry, and a newly authorized Step 4 dispatch): F2 line 1346 → reorder the expected literal to sorted order; F2 line 886 → assert the exact fixture values (`/home/a/.pm2`, `/home/b/.pm2`) are absent instead of the `.pm2` fragment. No F1 change is indicated. §11 placeholders are deliberately **not** filled from a failed run. Evidence preserved outside the repository at `C:\Users\knlee\aisb-k4-evidence\pm2-recovery-verify\run-35428399072\` (`run.json`, `run-api.json`, `artifacts-api.json`, `job-log.txt`, the downloaded artifact, `STEP4-VERDICT.txt` with per-file sha256). Run `35427018420` and §14.4 / §14.5 are retained as history. No lock; no implementation edit; no gate weakened; occupancy EMPTY throughout (Step 4 writes no F1–F6 path, so no lane was admitted and no HOTFILE lease taken); sidecar not edited. `EXEC-01C6A` stays `NOT_READY`.

### 14.7 F2/F4 assertion correction (2026-09-19; Keith-authorized; behavioural verification still pending)

Keith authorized the bounded F2/F4 correction identified by failed run `35428399072` at HEAD `c3bc92d41d122b4a8f11c32b9582c84b2e8ea898`. Preserved run evidence (`STEP4-VERDICT.txt`, cleared `unittest.log`) names both failures as F2 test-authoring defects. Transient admission (Lane 1 ACTIVE; HOTFILE leases on the frozen F1–F6 set matching the candidate write set; candidate `status=ADMITTED` / `admissionUncertain=false`; `occupancyHash=sha256:910af2507a083e2f370f99d447be0d8e71fe65c08db06a2dd7f06dc1b36d2bc1`; validator PASS) preceded the edit; Lane 1 was released EMPTY afterwards (candidate `status=READY` / `admissionUncertain=true`; `occupancyHash=sha256:942ff6798903e6f79e92aca2e8641dfcf7d4e19903c94c3429b13f2c37e5ec3d`).

F2 `ops/pm2-recovery-verify/tests/test_compare_dual_env.py` only:

1. `test_t10_output_mode_0600_and_output_exists` — expected `sorted(os.listdir(...))` literal reordered so `reference.json` precedes `report.json`. Directory membership and cardinality (five files) unchanged.
2. `test_t6_pm2_home_and_daemon_pid_checks` — both over-broad `assertNotIn(".pm2", …)` sentinels replaced with boolean assertions on the complete fixture paths, with fixed messages so a failure cannot print captured output:
   - mismatch invocation: `/home/a/.pm2` and `/home/b/.pm2` each checked in stdout and in stderr;
   - observation-only invocation: `/home/b/.pm2` checked in stdout and in stderr (`err` now captured).
