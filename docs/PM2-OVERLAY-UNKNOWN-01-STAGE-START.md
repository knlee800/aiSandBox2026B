...
| K4 | Select and authorize the verification environment (§5.2) and its resource declaration, if any | **SELECTED 2026-09-18** (Keith: standard GitHub-hosted ubuntu-24.04, fake-only, `workflow_dispatch` only; bounded scope extension to `.github/workflows/pm2-overlay-unknown-verify.yml`; no catalogue resource — not LOCAL-RUNTIME / STAGING / PROVIDER-LIVE; `runtimeNeeds=[]`; workflow PREPARED, NOT dispatched; §12). First remote run 35327293618 FAILED; corrections committed `4ead7cfc4d6b6b64f628a0420448d1c98a140cc1` (§13). **Rerun 35342598626: TEST_SUITE=PASS_WITH_ONE_EXPECTED_SKIP / WORKFLOW=FAIL_REPORTING; two reporting fixes WRITTEN / NOT YET REMOTELY VERIFIED (§14); complete suite + all workflow gates must rerun after commit** |
...
## 14. Step 4c — K4 rerun TEST_SUITE=PASS_WITH_ONE_EXPECTED_SKIP / WORKFLOW=FAIL_REPORTING; reporting fixes WRITTEN / NOT YET REMOTELY VERIFIED — 2026-09-18

Keith committed the Step 4b corrections as `4ead7cfc4d6b6b64f628a0420448d1c98a140cc1` and dispatched the workflow once. This window read that run via `gh` (no bundle code executed locally), preserved it outside the repository, and wrote **two workflow reporting corrections only**. Bundle source, tests, manifests, sidecar, `.gitattributes`, predecessors, flags, and EXEC-01C6A are unchanged. K5 remains NOT AUTHORIZED. No new task, no scope expansion, no gate weakened.

Recorded distinction (this run is **not** K5 evidence and **not** a lock):

- `TEST_SUITE=PASS_WITH_ONE_EXPECTED_SKIP`
- `WORKFLOW=FAIL_REPORTING`
- `FIXES=WRITTEN_NOT_YET_REMOTELY_VERIFIED`

### 14.1 Rerun evidence (preserved; not acceptance)

- Run `35342598626` (attempt 1, `workflow_dispatch` by `knlee800`, `refs/heads/main`), `checkout_sha = dispatch_sha = 4ead7cfc4d6b6b64f628a0420448d1c98a140cc1`; job `fake-only verification (ubuntu-24.04, Python 3.12)` started 2026-09-18T12:02:32Z, completed 2026-09-18T12:02:50Z; overall `conclusion=failure`.
- Steps: checkout / Python / identity / **preflight PASS** / **A PASS** / **B PASS** / **C FAIL** (`PIPESTATUS[1]: unbound variable` after the suite had already printed its summary) / **D FAIL** (`RESULT_GATE_FAIL`: `no 'Ran N tests' summary line`, `unittest exit code -1`) / manifest-writer copy PASS / **E PASS** (`TRACKED_BYTES_UNCHANGED`) / summary + artifact uploaded.
- Suite (from `unittest.log`, written by `tee` before the Step C shell died): `Ran 128 tests in 9.307s` — `OK (skipped=1)`. Parsed statuses in `test-summary.json`: `ok=127`, `FAIL=0`, `ERROR=0`, `skipped=1` (only `tests.test_operator_bundle.AdapterBoundaryTests.test_bash_stop_wrapper_if_present`, the allowed skip). Required Linux-specific cases **19/19 PASS**, including the three capture fixtures, T5, T7, T7b, and T13. `failures=0 errors=0`. No warning-output tests. `unittest.exit` / `unittest.timing` absent (Step C never wrote them). Parser `unittest_exit: -1`, `ran: -1`, `elapsed_s: null`, while `final_status_line` was correctly `OK (skipped=1)` and `expected_from_ast: 128`.
- Overall workflow remains FAILED because of the two reporting defects, not because of a test failure.
- Preserved outside the repository at `C:\Users\knlee\aisb-k4-evidence\run-35342598626\` (artifact `pm2-overlay-unknown-verify-35342598626-1` + `full-job.log`). Not copied into the repository. Run 35327293618 evidence remains at `C:\Users\knlee\aisb-k4-evidence\run-35327293618\`.

### 14.2 Root causes and corrections (each WRITTEN / NOT YET REMOTELY VERIFIED)

1. **Step C pipeline-status capture — `PIPESTATUS[1]: unbound variable`.** Trace: GitHub invokes `bash -e -o pipefail` and `-u`. After `set +e`, the two-command pipeline `timeout … unittest | tee` completed (suite exit 0; `tee` wrote `unittest.log`). The next commands were `rc="${PIPESTATUS[0]}"` then `tee_rc="${PIPESTATUS[1]}"`. The first assignment is itself a command, so `PIPESTATUS` is reset to that assignment's status (a one-element array). Under `-u`, `${PIPESTATUS[1]}` is unbound; the shell exited at that line; `unittest.exit` / `unittest.timing` were never written; Step C failed even though the suite had passed. **Correction:** immediately after the pipeline, with no intervening command: `pipeline_status=("${PIPESTATUS[@]}")` then `rc="${pipeline_status[0]}"` and `tee_rc="${pipeline_status[1]}"`. `set +e` remains around the pipeline; `set -e` is restored afterward; exit/timing evidence is written unconditionally; the step still fails with the unittest status, or with tee's status if tee failed. No `continue-on-error`. Isolation preflight, skip allowlist, and result gates unchanged.

2. **Step D summary matching — missing `Ran N` line.** Trace: `RAN = re.compile(r"^Ran (\d+) tests? in ([\d.]+)s$")` is start/end anchored. `RAN.search(text)` was applied to the whole multiline log **without** `re.MULTILINE`, so `^`/`$` bound the entire blob, not each line. `unittest.log` contained `Ran 128 tests in 9.307s` (confirmed in the artifact); the parser reported `no 'Ran N tests' summary line` and `ran: -1`. The per-line verbose parser already used `RAN.match(line)` to end the body and had already counted 128 statuses. **Correction:** `RAN.fullmatch` over the existing `splitlines()` output; require **exactly one** summary match; reject missing (`no 'Ran N tests' summary line`) or duplicate (`duplicate 'Ran N tests' summary lines: N`) summaries. When exactly one match exists, comparison against the AST count (`ran_n != expected`), parsed status count (`len(status) != ran_n`), required cases, final `OK`/`FAILED` line, skip allowlist, and actual `unittest.exit` code are unchanged. No gate weakened; no test changed; no allowed skip added.

### 14.3 Changed paths

- Workflow only: `.github/workflows/pm2-overlay-unknown-verify.yml` (Step C PIPESTATUS array capture; Step D `RAN.fullmatch` over `splitlines()` + exactly-one summary). Trigger, runner, permissions, pins, preflight, A, B, manifest-writer copy, E, artifact, required-test list, skip allowlist, and failure/count gates unchanged.
- Governance: this §14 + status header + K4 row; `TASKS.md` this task's current board fields; canonical body; `SATURATION_PROOF.json` by the validator only.
- **Unchanged:** all 40 files under `ops/aisb-01c6a-operator-bundle/`, manifests, sidecar, `.gitattributes`, predecessor bodies, EXEC-01C6A.

### 14.4 Remaining limitations

- The two reporting fixes are **WRITTEN / NOT YET REMOTELY VERIFIED**. After Keith commits, the next remote run must execute the **complete suite and all workflow gates** (`gh workflow run pm2-overlay-unknown-verify.yml --repo knlee800/aiSandBox2026B --ref main`). A green workflow is still fake-only LOCAL-TESTS evidence, not K5 acceptance.
- `TEST_SUITE=PASS_WITH_ONE_EXPECTED_SKIP` on run 35342598626 is preserved as evidence that the K3/4b behavioural corrections held on GitHub-hosted Linux. It does **not** close K4 while `WORKFLOW=FAIL_REPORTING`.
- Nothing here establishes F1–F5, P1/P4–P7, host CLEAN, EXEC-01C6A reopen readiness, staging transfer, a canary, or K5 acceptance/lock.

### 14.5 Control-plane end state and activity ledger (Step 4c window, 2026-09-18)

GOVERNANCE acquired transiently then released UNOWNED. Occupancy EMPTY; Lane 3 DISABLED; `admissionUncertain=true`; not in S; task NOT LOCKED; EXEC-01C6A `startCondition=NOT_READY` unchanged; Builder gate LEFT ON; `stagingAuthorized=false`; `STAGING_EXECUTION_AUTHORIZED=NO`. All changes uncommitted pending Keith's Git.

LIVE=0, SSH=0, staging=0, AWS=0, provider=0, credits=0, runtime=0, Docker=0, Postgres=0, Redis=0, PM2=0, flags=0, key creation=0, canary submission=0, browser=0, subagents=0, Git add/commit/push/branch/worktree=0, workflow dispatched=0, Actions settings/billing changed=0, secrets created=0, bundle files edited=0 (40/40 identical to `4ead7cfc4d6b6b64f628a0420448d1c98a140cc1`), `.gitattributes` edited=0, archives mutated=0, Python executed=0, tests executed=0 except lane-capacity validator, mocks=0, builds=0, installs=0, workflow emulation=0, network reads=`gh run view/download` of run 35342598626 only, predecessor body edits=0, EXEC-01C6A changes=0, Lane admission=0, Lane 3 enablement=0, r3 archive produced=0, successor registration=0, K5 acceptance/lock=0, allowed skips added=0, preflight relaxed=0, tests changed=0.
