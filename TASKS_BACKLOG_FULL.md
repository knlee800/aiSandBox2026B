...
- This blocks Area 3 (Session Lifecycle Flow) validation steps 3.6??.9 and Area 6 prerequisites
...
1. Reuse the existing history/control surface and active-session checkpoint list already present (from TASK-78B / Phase 79??0 preserved behavior)
...
4. Preserve the working TASK-84A??4E submit, thread, error, and refresh-persistence flow
5. Keep the change frontend-only and tightly scoped
6. Preserve all completed Phase 83 and Phase 84 behavior
7. Do not expand into broader chat redesign
8. Focused frontend tests for this slice
...
A real usage attempt created previewable files, but the UI still shows:
- Preview unavailable
...
PREV-01-01 isolated the failure clearly:
- AI-created `index.html` exists in the real session workspace
- file reads succeed
- preview status remains unavailable because it only trusts preview-service active state
- `/api/preview/:sessionId/start` fails because it checks a workspace source that is empty while the session container actually has the files
...
PREV-01-02 proved backend preview start/status/proxy now work for AI-created static HTML. But the real UI still shows:
- Preview unavailable
- Refresh button only
- No visible way to call preview start

The backend path is functional; the missing piece is a frontend action that surfaces it to the user.
...
- Fix is documented clearly in `docs/PREV-01-03-CHECKPOINT.md`
...
**Task ID:** PREV-02-01
**Family:** PREV-02 (Preview Post-Persistence Regression)
**Status:** COMPLETE and LOCKED
...
**Why this exists:**
Real usage now shows:
- project/snapshot persistence works after restart
- preview returns HTTP 500 / Internal server error
- preview previously worked after PREV-01 fixes

**Bounded scope only:**
- reproduce preview 500 on a session with valid files
- inspect preview status/start/proxy path
- inspect api-gateway and container-manager logs for the exact error
- determine whether failure is in preview start, status, proxy, static file serving, container lookup, or workspace path mapping
- identify exact failing stage clearly
...
**Why this exists:**
PREV-02-01 isolated the defect:
- static preview detection accepts any *.html file
- preview starts as Static HTML
- proxy root path always resolves to index.html
- if session has hello.html but no index.html, proxy fails with 500

**Bounded scope only:**
- inspect static HTML preview detection and proxy path handling
- make detection/proxy entry file behavior consistent
- prefer either requiring index.html for static preview or selecting a valid detected HTML entry file
- preserve existing index.html behavior
- verify hello.html-only case no longer produces misleading start-success + proxy-500
- document exact fix
...
Real usage exposed two core usability problems:
- A saved project does not appear to open into a usable workspace
- Public Projects shows failed to load / unauthorized
...
PROJ-01-01 isolated the failure:
- `GET /api/projects/public` is being caught by the wrong route
- `ProjectsController @Get(':id')` intercepts `/public`
- This causes unauthorized/broken public-projects behavior
...
- Fix is documented clearly in `docs/PROJ-01-02-CHECKPOINT.md`
...
Real usage still shows:
- Project open success message appears
- But no files appear in the file tree/editor afterward

This means the remaining failing stage is still unresolved in the real UI path.
...
PROJ-01-05 isolated the remaining failure:
- Frontend project open sends `snapshotId` from a global latest user snapshot
- Not the latest snapshot belonging to the selected project
- This can restore an unrelated empty snapshot and make the workspace appear blank
...
- Fix is documented clearly in `docs/PROJ-01-06-CHECKPOINT.md`
...
Real user behavior still reproduces:
- save project
- open project in another session
- no files appear

This means either:
- the save flow is not actually persisting project content the way the user expects, or
- the real UI flow still differs from the already-tested API-equivalent path.
...
PROJ-01-07 isolated the remaining real-user failure:
- `Create Project` saves project metadata only
- no project-scoped snapshot exists unless the user separately clicks `Save Snapshot`
- later `Open Project` can only bind an empty project unless a snapshot exists
- this violates normal user expectations for "saved project"
...
- Fix is documented clearly in `docs/PROJ-01-08-CHECKPOINT.md`
...
Real usage shows project download currently fails with:
...
PROJ-01-14 isolated the failure clearly:
- root export works
- nested recursion builds /src, /src/... absolute paths
- container-manager rejects those as outside /workspace
- download fails for projects with nested folders
...
- user must manually refresh browser to see restored files

**Bounded scope only:**
- reproduce the exact real UI flow again
- inspect current post-open reload path and rendered file-tree state
- compare current behavior against the prior fixes
- identify the exact remaining failing stage clearly
...
**Why this exists:**
PROJ-01-16 isolated the real remaining failure:
- workspaceSnapshots is cleared on session switch
- snapshot reload is asynchronous
- Open Project can run before snapshots reload
- resolveProjectScopedLatestSnapshotId sees []
- project open takes associate-only path instead of restore path
- workspace remains empty until browser reload

**Bounded scope only:**
- inspect project-open snapshot resolution
- make Open Project resolve the project-scoped latest snapshot from a fresh/current source at open time
- preserve explicit snapshot behavior if present
- preserve safe bind-only fallback only when the selected project truly has no snapshots
- verify Open Project restores content even immediately after session switch, without browser reload
...
Real usage still shows:
- project open succeeds
- restored files exist after open
- but the editor panel does not update automatically
- user-visible editor state remains stale or empty

**Bounded scope only:**
- reproduce the real project-open UI flow
- inspect file tree state after open
- inspect selected file state after open
- inspect editor content state after open
- inspect whether a file is auto-selected or whether content reload is skipped
- identify the exact remaining failing stage clearly
- no fix in this task unless a trivially obvious diagnostic correction is absolutely required, which normally should not be done
...
**Why this exists:**
PROJ-01-18 isolated the remaining editor-panel failure:
- file tree refresh works
- selected file is set correctly
- editor content load starts
- AI file-action coherence effect fires from stale applied entries
- coherence reload increments fileContentRequestIdRef
- project-open editor content request fails staleness check
- editor never reaches ready

**Bounded scope only:**
- inspect the AI file-action coherence effect and project-open editor load sequence
- prevent stale coherence work from invalidating project-open editor content loading
- preserve normal AI file-action coherence behavior
- preserve project-open file tree/editor behavior
- verify project open shows editor content without browser reload
...
**Why this exists:**
Real usage shows this exact pattern:
- Session A has files
- project is saved/created
- opening the saved project into Session B before restart works
- after restarting all servers, opening the same saved project into Session C shows no files

This means immediate cross-session restore works, but persistent restore across server/container restart does not.

**Bounded scope only:**
- reproduce the exact before-restart / after-restart flow
- inspect where project snapshot metadata is stored
- inspect where snapshot/archive file content is stored
- inspect whether snapshot content survives `docker compose down` / restart
- inspect whether restore after restart uses the correct snapshot
- distinguish clearly:
  - project metadata persistence
  - snapshot metadata persistence
  - snapshot/archive content persistence
  - restore into new session after restart
...
**Why this exists:**
PROJ-01-20 isolated the critical persistence failure:
- project DB rows survive restart
- snapshot files are written to `/snapshot-store/` inside the api-gateway container writable layer
- docker-compose.prod.yml does not mount a persistent volume for `/snapshot-store/`
- `docker compose down` removes the container and deletes the snapshot files
- saved projects then open empty after restart

**Bounded scope only:**
- inspect snapshot storage path/config
- add persistent Docker volume or host-mounted storage for `/snapshot-store/`
- apply to the relevant compose files used by local/prod-style stack
- preserve existing snapshot read/write behavior
- verify saved project restores before and after `docker compose down` / restart
...
**Why this exists:**
Real usage still shows:
- saved project persistence now works
- opening project can restore content after restart
- but files still do not automatically appear/load in the UI after Open Project

**Bounded scope only:**
- reproduce the exact real project-open UI flow
- inspect whether backend restore succeeds
- inspect whether file tree reload is called
- inspect whether file tree reload response contains files
- inspect whether selected file/editor content state is updated
- inspect whether rendered file tree/editor receives the updated state
- identify the exact remaining failing stage clearly
- no fix in this task unless a trivially obvious diagnostic correction is absolutely required
...
- clear distinction is made between backend restore, file-tree reload, selected-file state, editor-content state, and rendered UI
- issue is narrowed enough for one bounded follow-up fix task
- no unrelated work is mixed in
...
**Why this exists:**
PROJ-01-22 isolated the remaining failure:
- backend restore works
- file-tree reload response contains files
- selected file/editor content state is structurally set
- but the selectedSessionId effect at L463 fires during the async Open Project handler
- it clears projectActionState, selectedProjectId, workspaceProjects, and launches concurrent async operations
- this races with the handler and leaves the rendered UI stale/empty

**Bounded scope only:**
- inspect the selectedSessionId effect at L463 and project-open handler interaction
- suppress or coordinate project-state reset during an intentional project-open session transition
- preserve normal session-switch behavior outside project-open
- preserve existing file reload/editor behavior
- verify Open Project loads files automatically without browser reload
...
**Implementation plan:** `docs/PROJ-03-01-IMPLEMENTATION-PLAN.md`
...
**Why this exists:**
All Phase A slices must be gated behind a kill-switch so the product stays stable while incremental UX changes are validated. A centralized copy bundle prevents ad-hoc string literals from scattering across components.
...
- No auth work, no operator console work

**Acceptance criteria:**
- Flag defaults to `false` in all environments
- Flag is flippable in dev via env var (`NEXT_PUBLIC_PROJECT_FIRST_UX=true`) without code change
- Copy bundle exports all strings the design and plan identify
...
**Dependencies:** None (first Phase A slice)
...
**Why this exists:**
The approved design requires new top-level routes and relabeled navigation. This slice is the structural prerequisite for all further Phase A slices. Adding routes and labels behind the flag first ensures the shell is testable and revertible before any content changes.
...
- No sessions list removal (that is A.2, not registered in this batch)
- No history tab, no share modal, no gallery data fetching
- No backend changes, no auth changes
...
**Dependencies:** PROJ-03-A0 must be complete first
...
**Objective:**
Replace all user-visible raw session/container lifecycle strings (e.g. "session expired", "container exited", "session disconnected") with the recovery vocabulary from the PROJ-03-A0 copy bundle, and surface a primary "Reopen project" action that wraps the existing project-open flow.

**Why this exists:**
The current UI exposes internal runtime concepts to users in error/status messages. This slice makes the user surface project-first at the vocabulary level while leaving all underlying logic untouched.
...
**Non-goals:**
- No change to underlying session lifecycle logic
...
**Acceptance criteria:**
- Flag on: no user-visible string exposes raw container/session/runtime mechanics
- All recovery paths present "Reopen project" as the primary action
- "Reopen project" correctly invokes the existing project-open flow (PROJ-02-01 hydration path)
- Flag off: today's string behavior is unchanged
...
- Stop-session flow (OPS-01-04) is not affected
...
**Why this exists:**
The approved design (Section 3.3) requires an Advanced drawer as the landing zone for demoted session controls. Building and locking the drawer shell as a separate slice ensures the structural foundation is independently verifiable before any live control (stop-session) is relocated into it in A2b.
...
**Acceptance criteria:**
- Flag on: Advanced drawer present, collapsed by default, expands correctly; session ID readable and copyable; status indicator visible
...
**Invariants explicitly preserved:**
- `PROJECT_FIRST_UX` remains kill switch; flag off is unchanged
- Workspace layout and overflow must not regress
...
**Why this exists:**
Once the Advanced drawer shell is locked (A2a), this slice completes the session demotion work from the approved design (Section 3.2 / 3.3): sessions disappear from primary nav and stop-session moves to the Advanced fallback surface, preserving full functionality without exposing session machinery to normal users.
...
- Flag off: sessions list and stop-session in their current locations, unchanged
- Existing workspace-shell tests pass; new flag-on tests verify stop-session in Advanced drawer and sessions list absent from primary
- Typecheck clean; no lint errors introduced on changed files
...
**Bounded scope:**
- Frontend only
- New/extended tests in `frontend/components/workspace/workspace-shell.test.tsx`
...
- Short audit of recovery surfaces (covered vs uncovered) captured in the checkpoint
...
**Dependencies:** PROJ-03-B2b (COMPLETE and LOCKED)
...
**Why this exists:**
Phase C begins with a purely read-only history surface so that any regression is local to the display layer and cannot affect the open/restore path. Writes (autosave triggers, named save) come in C2 only after C1 is observed stable.
...
**Why this exists:**
C1a added the read-only history panel. Before a visible Restore CTA can be wired up (C1b-cta), the handler that performs the actual restore operation must exist as a separately validated, locked primitive. Splitting handler from UI mirrors the B4a/B4b pattern and ensures the restore path can be reviewed and locked before any user-visible trigger is added. This slice is handler-only.
...
- Additive changes in `frontend/components/workspace/workspace-shell.tsx`:
  ...
  - Otherwise calls `props.onSaveNamedProjectSnapshot(name)` with the prompted name
...
**Behavior (when `PROJECT_FIRST_UX` is true):**
- Automatic version labels are enriched with deterministic heuristic context available at save time
- Labels become more distinguishable than the current source-only labels
- Manual named saves continue to render the user-supplied name unchanged
- Older source-tagged entries continue to render the existing source-based labels
- Older unlabeled entries continue to fall back to `'Saved version'`
- No AI call, no async naming pipeline
- No backend/API/schema changes
- No new route
- No history redesign
...
**Risks and invariants:**
- Keep this deterministic and heuristic-only; do not introduce AI dependency or async naming flow
- Preserve backward compatibility with existing stored labels
- Do not change backend behavior or history semantics
- `PROJECT_FIRST_UX` remains the kill-switch posture
- Preserve all existing save/restore behavior; only improve label clarity
- No regression to: project-open hydration / restore discipline (PROJ-02-01); snapshot/history persistence behavior (PROJ-01-21); `.git/` exclusion from snapshots/restores (PROJ-02-03); static preview `/workspace/index.html` rule (PREV-02-02); stop-session cleanup behavior (OPS-01-04)
...
**Why this exists:**
D1d introduced session reuse based on the in-memory `sessions` React state. That state is only refreshed on `loadSessions()` calls. Between the last refresh and a "Open Project" / "Resume" click, the server may have terminated the session (e.g. idle_timeout), making the in-memory state stale. The frontend then reuses the terminated session, and the backend rejects it with 410 Gone. The fix is to refresh sessions immediately before the reuse decision, not to add retry-on-410 fallback logic.
...
**Behavior:**
- Before calling `openProjectInFreshSession` for "Open Project" and "Resume latest project", always refresh sessions from the backend first
- The reuse check in the helper then sees the current server state
- If the previously-reusable session was terminated, it will have `terminatedAt` set in the refreshed data, `isUsableSession` will return false, and the helper falls through to `createWorkspaceSession` as intended
- Explicit snapshot restore path remains unchanged and always-fresh
...
**Risks and invariants:**
- Keep this to stale-state correction only; do not broaden into retry logic or backend changes
- Preserve explicit restore always-fresh behavior
- Preserve hydration/open flow after reuse
- Preserve tab-isolated selection behavior from D0d
- Preserve `projectOpenInProgressRef` guard and all open/restore invariants
- Preserve `.git` exclusion, autosave timing, and stop-session cleanup semantics
- `PROJECT_FIRST_UX` remains the kill-switch posture
- No regression to: project-open hydration / restore discipline (PROJ-02-01); snapshot/history persistence (PROJ-01-21); `.git/` exclusion (PROJ-02-03); static preview `/workspace/index.html` rule (PREV-02-02); stop-session cleanup (OPS-01-04)
...
After OPS-01-02, Docker still appears to contain many session containers. We need to distinguish:
- old leaked containers left behind before the fix
- versus new post-fix stop/terminate flows still failing to remove containers

**Bounded scope only:**

- inspect current sandbox-session-* containers
- compare creation/start times where possible
...
**Task ID:** OPS-LOCAL-SESSION-LIMITS
...
**Non-goals:**
- No change to token quotas (`MAX_TOKENS_PER_24H`)
...
- Local docker compose overrides to 1000000 / 1000000
- Active sessions guard uses overridden value correctly
- 24h sessions guard uses overridden value correctly
- Relevant tests pass (updated if assertions on 5/20 break)
- `api-gateway` rebuilds cleanly
- `docker compose -f docker-compose.prod.yml config` parses cleanly

**Risks and invariants:**
- Local-testing only; do not treat as production policy
- Use large finite integers (`1_000_000`), never `Infinity`
- Preserve existing `SessionQuotaGuard` enforcement paths and behavior
- Do not affect token quota handling
- Keep the diff small and reversible via `git restore`
...
**Acceptance checks:**
- `docker-compose.prod.yml` parses cleanly (`docker compose -f docker-compose.prod.yml config` exits 0)
- `api-gateway` rebuilds cleanly
- `docker compose ... up -d api-gateway` starts cleanly
- New `api-gateway` environment contains `JWT_EXPIRES_IN=30d`
- Newly issued local tokens use the longer lifetime (verified at login)
- No code changes required

**Risks and invariants:**
- Local-testing only; do not treat 30d access tokens as production policy
- Existing tokens issued before the change remain 15m short-lived; user must log in once after the rebuild to receive a new token
- No auth architecture change; frontend logout-on-401 behavior is untouched
- Keep the diff tiny and reversible via `git restore`
...
Real usage still shows the project box is confusing:
- users are unsure whether they must choose private/public when creating a project
- private project management and public project browsing are mixed together
- the current dense block does not clearly communicate the normal workflow
...
- Fix is documented clearly in `docs/UX-02-02-CHECKPOINT.md`
...
**V1 Workspace definition:**

A Workspace is a named, user-owned personal organizational container for projects. In v1 it is personal-only (single owner, no members, no shared access, no billing). Each user has exactly one default Personal workspace. Projects belong to exactly one workspace. Sessions remain attached to projects, not workspaces.

**Bounded scope:**

- Backend/schema only
- Allowed files/surfaces:
  - new `Workspace` TypeORM entity (e.g. `services/api-gateway/src/entities/workspace.entity.ts`)
  - Workspace module foundations as needed for entity/schema wiring only (not full service/controller)
  - `Project` entity update to add nullable `workspaceId` FK and TypeORM relation to Workspace
...
|---|---|---|
| id | uuid / serial | PK |
| userId | uuid / int | FK to users, not null |
| name | varchar | not null |
| slug | varchar | not null, URL-safe |
...
- unique `(userId, slug)`
- index on `userId`
- index on `(userId, isDefault)` for fast default-workspace lookup

**Migration behavior:**
1. Create `workspaces` table with above fields
2. Add nullable `workspace_id` column to `projects` table
3. Add FK constraint `projects.workspace_id -> workspaces.id`
4. Backfill: for each distinct `userId` in `projects`, insert one default Personal workspace if not already present, then `UPDATE projects SET workspace_id = <defaultWorkspaceId> WHERE userId = <userId> AND workspace_id IS NULL`
5. Backfill must be idempotent (safe to re-run; no duplicate default workspaces, no double-assignment)

**Non-goals for this slice:**

- No frontend workspace selector
- No workspace CRUD API (list / create / rename / delete endpoints)
- No workspace filtering in project list endpoint
...
- No NOT NULL enforcement on `workspace_id` in this slice (deferred to later slice after all paths create projects with a workspace)
- No D1/PROJ-03 work
- No Phase D/E or other unrelated work

**Acceptance checks:**

- New `workspaces` table exists with agreed v1 fields and constraints
- `projects` table has nullable `workspace_id` column with FK wiring
- TypeORM `Workspace` entity present and consistent with migration
- TypeORM `Project` entity has `workspaceId` nullable field and `workspace` relation
...
- Migration is idempotent/safe for rerun
- Existing behavior outside workspace awareness remains unchanged
- Relevant backend typecheck passes; affected tests pass
- No migration scope beyond this bounded foundation
...
- Keep entity/field naming consistent with future expansion to members/roles/billing
- Do not introduce workspace-awareness into any existing endpoint behavior in this slice
...
**Reference:** See TASKS.md -> AI-WS-05 for active-task summary.
